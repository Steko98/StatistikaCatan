...
async function getIgre(sifra) {
  return await HttpService.get("Turnir/Igre/" + sifra)
    ...
    .catch(() => {
      return {
        greska: true,
        poruka: "Problem kod dohvaćanja igara na turniru",
      };
    });
}
...
async function traziTurnir(uvjet) {
  return await HttpService.get("Turnir/trazi/" + uvjet)
    ...
    .catch((e) => {
      return { greska: true, poruka: "Problem kod traženja turnira" };
    });
}
...
async function getDetaljiTurnir(sifra) {
  return await HttpService.get("Turnir/" + sifra + "/detalji")
    ...
    .catch(() => {
      return { greska: true, poruka: "Problem kod dohvaćanja detalja turnira" };
    });
}
